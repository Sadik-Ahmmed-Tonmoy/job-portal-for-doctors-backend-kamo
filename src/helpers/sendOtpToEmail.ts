...
import sendEmailToBrevo from "./sendMailBrevo";
import sendEmail from "./sendMailBrevo";

export const sendOtpToGmail = async (existingUser: any,otp:string) => {
  ...
  // sentEmailUtility(existingUser.email, subject, html);
 const result=await sentEmailUtility(existingUser.email, subject, html);
 console.log(result)
